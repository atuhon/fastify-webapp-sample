...
//item
  ...
  server.post(
    ...
    async (request, reply) => {
      const client = await server.pg.connect();
      const { itemId } = request.params;
      const { name, description, price, default_inventory, inventory } =
        request.body;
      const item = (
        await client.query(
  ...
  server.get("/items/:itemId/favorite", async (request, reply) => {
    if (!request.user) {
  ...
  server.get("/items/:itemId/unfavorite", async (request, reply) => {
    if (!request.user) {
      await reply.redirect(302, "/items");
    }
    const client = await server.pg.connect();
